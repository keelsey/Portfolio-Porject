...
select location, population, date, max(total_cases) as highestcovidcase, round(max((total_cases::decimal/population)) * 100, 2) as "Percentage population infected"
from covid_deaths
...
--VIEW 1
create view Total_Deaths_Percentage as
select /*date,*/ sum(new_cases) as "Total case", sum(new_deaths) as "Total Deaths", round((sum(new_deaths)::decimal/sum(new_cases))*100, 2) as "Death Percentage"
from covid_deaths
where continent is not null
--group by date
...
--VIEW 2
create view Total_Deaths_by_loc as 
select location, max(total_deaths) as "Total Deaths"
from covid_deaths
...
--VIEW 3
create view Percentage_Pop_Infected as 
...
--VIEW 5
create view population_infected_date as
select location, population, date, max(total_cases) as highestcovidcase, round(max((total_cases::decimal/population)) * 100, 2) as "Percentage population infected"
from covid_deaths
group by location, population, date
order by "Percentage population infected" desc nulls last;

drop view population_infected_date;
